...
public class AlbumTest {
    ...
    private static final String TIPO_ALBUM = "selo";

    @Before
    public void setUp() {
        this.repositorioColecionaveis = new Repositorio("album_copa2014", TAMANHO_DO_ALBUM,TIPO_ALBUM);
    ...
    private void popularAlbum(int[] posicoesDesejadas) {
        Pacotinho pacote = new Pacotinho(this.repositorioColecionaveis, posicoesDesejadas, TIPO_ALBUM);
    ...
    public void testarAutoCompletar() {
        ...
        while (albumColecionaveis.getQuantItensColados() < minimoFigurinhasColadasParaAutoCompletar) {
            Pacotinho novoPacotinho = new Pacotinho(
                    this.repositorioColecionaveis, ITENS_POR_PACOTE, TIPO_ALBUM);  // aleatório
        ...
        albumColecionaveis.autoCompletar();
        assertEquals("O colecionavel deve estar completo após uma chamada válida ao auto-completar " +
                        "(isto é, após o percentual mínimo de figurinhas já ter sido obtido)",
                0, albumColecionaveis.getQuantItensFaltantes());
    }

    @Test
    public void testarGetItemColado() {
        popularAlbum(new int[] {1, 2, 3});
        Colecionavel figurinha = albumColecionaveis.getItemColado(2);
